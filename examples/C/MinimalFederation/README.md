# Minimal C Federation Example

This directory contains a minimal C federation example demonstrating basic federated execution concepts in Lingua Franca.
...
```bash
cd examples/C/MinimalFederation/src
lfc CountPrint.lf
```

This will generate:
- Federate C binaries for Count and Print reactors
- RTI (Runtime Infrastructure) binary
- Launch script in `examples/C/MinimalFederation/bin/CountPrint`

## Execution

To run the federation:
```bash
cd examples/C/MinimalFederation/bin
./CountPrint
```
...
- **Logical Time**: Deterministic timing across distributed federates using `lf_time_logical_elapsed()`
